...
static int __devinit gm_init_metric(struct gm_hwinfo *hw, int m_idx)
{
	...
	/*
	 * If this is a triggerable metric, then hook it up
	 * to the LED subsystem
	 */
	if (metric->is_triggered) {
		metric->led.name = metric->name;
		metric->led.brightness_set = gm_led_brightness_set;
		metric->led.max_brightness = INT_MAX;
		err = led_classdev_register(&hw->gm_dev->dev, &metric->led);
		if (err) {
			pr_err("Failed to register led.");
			goto err_unreg_dma;
		}
	}
#endif
...
err_unreg_dma:
	if (gm_do_cmd(hw, m_idx, GMETRICS_CMD_CLRMETRICDMA))
		pr_err("Failed to clear metric dma addr\n");
	return err;
err_destroy_bin:
